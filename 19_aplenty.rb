...
# This parsing works, but it's using workflows.frozen? as a signal for whether we're in the parts section
# maybe it'd be better to split \n\n first?
workflows = {}
puts ARGF.sum { |line|
  line.chomp!
  if line.empty?
  ...
  if workflows.frozen?
    ...
    part = brackets(line)[1].to_h { |kv|
      k, v = kv.split(?=)
      [k.to_sym, Integer(v)..Integer(v)]
    }
    num_accept(workflows, 'in', part) * part.values.sum(&:begin)
  else
    name, rules = brackets(line)
    workflows[name] = rules.map(&method(:rule)).freeze
    0
  end
}
